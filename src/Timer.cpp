...
void Timer::calculateSunriseSunset()
{
    ...
    logDebug("Timer", "long %f", mLongitude);
    logDebug("Timer", "lati %f", mLatitude);
    ...
    logDebug("Timer", "Sunrise %.2i:%.2i (%f)", mSunrise.hour, mSunrise.minute, rise);
    logDebug("Timer", "Sunset  %.2i:%.2i (%f)", mSunset.hour, mSunset.minute, set);
}
...
void Timer::setDateTimeFromBus(tm *iDateTime)
{
    ...
    logDebug("Timer", "%i:%i:%i", getHour(), getMinute(), getSecond());
}
